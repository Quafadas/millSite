 ...
 *
 * This module enables building static documentation sites from Markdown sources,
 * with optional API documentation integration via Scaladoc. It uses the Helium theme
 * for styling and supports live reload functionality during development.
 *
 * ==Features==
 ...
 *
 * ==Directory Structure==
 * By default, documentation sources are expected in a `docs` subdirectory
 * relative to the module directory. The generated site will include:
 *  - Transformed Markdown files as HTML
 *  - API documentation under `/api/` (if enabled)
 *
 * @see [[laika.helium.Helium]] for theme configuration options
 * @see [[UnidocModule]] for API documentation generation
 */
...
trait LaikaModule extends Module:
  ...
  /** Sequence of Mill modules whose sources should be included in API documentation.
    *
    * Override this to include modules in the generated Scaladoc.
    * If empty, API documentation generation is skipped.
    *
    * @return Seq of JavaModule instances to document
    */
  ...
  /** Whether to include API documentation in the generated site.
    *
    * Returns `true` if [[laikaUnidocDeps]] is non-empty, meaning there are
    * modules to document. The API docs will be placed under `/api/` in the site.
    *
    * @return true if API documentation should be generated and included
    */
  def includeApi: Simple[Boolean] = Task(laikaUnidocDeps.nonEmpty)

  /** The source directory containing Markdown documentation files.
    *
    * Defaults to a `docs` subdirectory relative to the module directory.
    * Files in this directory (and subdirectories) will be processed by Laika.
    *
    * @return PathRef to the documentation source directory
    */
  ...
  /** The base URL for the generated site.
    ...
    *
    * @return the base URL string for the site
    */
  def baseUrl: Simple[String] = unidocs.unidocSourceUrl().getOrElse("!!!no path!!!")

  /** The URL of the project's source repository.
    *
    * Used to generate the GitHub icon link in the navigation bar.
    * Override this with your actual repository URL.
    *
  ...
  /** The latest version of the documented library.
    *
    * This value is made available in Laika templates as `version.latest`.
    * Override this to display the current version in your documentation.
    *
  ...
  /** Configuration values to inject into Laika templates.
    *
    * These key-value pairs are available in Laika's substitution references.
    * By default includes:
    *  - `version.latest` - the library version
    *  - `laika.siteBaseURL` - the base URL for the site
    *
    * @return sequence of (key, value) pairs for template substitution
    */
  ...
  /** Worker task that builds the Helium theme configuration.
    ...
    *
    * @return the configured Helium theme builder
    */
...
"""
  ...
  /** Stages the documentation site by copying sources and API docs to a build directory.
    ...
    *
    * @return PathRef to the staged site directory
    */
  def stageSite = Task {
  ...
  /** Generates the final HTML site from Markdown sources using Laika.
    ...
    *
    * @return PathRef to the generated site directory containing HTML files
    */
  def generateSite =
    Task {
      BuildCtx.withFilesystemCheckerDisabled {
