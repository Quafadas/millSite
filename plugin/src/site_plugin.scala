...
/** The main Mill module trait for generating documentation sites.
  *
  * `SiteModule` is the primary entry point for building documentation sites. It orchestrates [[MdocModule]],
  * [[LaikaModule]], and [[UnidocModule]] to produce a complete static site with:
...
trait SiteModule extends Module:
  ...
  /** Optional Scala.js module for interactive code examples.
    *
    * Override with a [[SiteJSModule]] instance to enable Scala.js code blocks in documentation. When `None`, JS code
    * blocks are disabled.
    */
  ...
  /** Topic for publishing live-reload update notifications.
    *
    * Used internally by the development server to trigger browser refresh via Server-Sent Events when documentation
    * changes.
    */
  ...
  /** Default directory for documentation source files.
    *
    * Points to `docs/` under the module directory. This is used as the input for Mdoc processing.
    */
  lazy val defaultInternalDocDir = super.moduleDir / "docs"

  /** Modules to include in API documentation generation.
    *
    * List the modules whose sources should be documented with Scaladoc. If empty, API documentation generation is
    * skipped.
    *
    * @return
    *   sequence of modules to document
    */
  ...
  /** Title for the generated API documentation.
    ...
    *
    * @return
    *   the API documentation title
    */
  def unidocTitle = Task("Unidoc Title Here")

  /** URL to the project's source repository.
    *
    * Used for the GitHub icon link in the site navigation bar. Override with your actual repository URL.
    *
    * @return
  ...
  /** The latest release version of your library.
    *
    * Defaults to the most recent Git tag (without the 'v' prefix). Used in site templates and available as the
  ...
  /** Resource directories to include in the generated site.
    *
    * Files in these directories are copied to the site output. Defaults to a `resources/` subdirectory under the
    * module.
    *
    * @return
    *   sequence of resource directory sources
    */
  def resources = Task.Sources(super.moduleDir / "resources")

  /** JVM arguments passed to forked processes.
    *
    * Override to add memory settings, system properties, or other JVM flags for Mdoc or other forked tool execution.
    *
    * @return
    *   sequence of JVM arguments
    */
  ...
  /** Optional path to an ES module import map file.
    ...
    *
    * @return
    *   optional PathRef to the import map JSON file
    */
  def pathToImportMap: T[Option[PathRef]] = None

  /** The Scala version for documentation compilation.
    *
    * This must be overridden - there is no default. The version should match the Scala version used by the library
    * being documented.
    *
    * @return
    *   the Scala version string (e.g., "3.3.1")
    */
  def scalaVersion: Simple[String] = Task(???) // force the user to set this themselves.

  /** Site variables for Mdoc substitution.
    *
    * Key-value pairs available in Markdown files using `@@KEY@@` syntax. By default includes `VERSION` set to
    * [[latestVersion]].
    *
    * @return
  ...
  /** ES module import mappings for Scala.js.
    *
    * Configures how ES module imports are resolved when using Scala.js code blocks. Useful for mapping Scala.js facade
    * imports to CDN URLs.
    *
    * @return
    *   sequence of import mappings
    */
  ...
  /** Returns the path to the generated site as a string.
    *
    * Convenience task that extracts the path from [[siteGen]] for use in server configuration.
    *
    * @return
    *   absolute path string to the site directory
    */
  ...
  /** Starts the live-reload development server.
    ...
    *
    * The server runs as a Mill worker task and stays alive until stopped. Access the site at
